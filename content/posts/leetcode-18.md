...
The trick that keeps the loops honest is the indexing: `i < j < k < l`. By forcing each loop to start one past the previous one, I never reuse an element and I never visit the same four positions in a different order. That alone removes a huge class of duplicates — the ones that come from *position* rather than *value*.
...
The wrinkle is that a `[]int` cannot be a map key — slices are not comparable in Go. The standard workaround is to turn each quadruplet into a `string`, which *is* comparable. I reached for `fmt.Sprintf` to build the key:
...
| Brute force (Step 1) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| Map dedup (Step 2) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| Two pointers + map (Step 3) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
...
| Map dedup (Step 2) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| Two pointers + map (Step 3) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| `strconv` key (Step 4) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
...
There are four places duplicates can sneak in — the two outer values and the two pointer values — and the cure is the same idea everywhere: only ever *start* a combination from the first occurrence of a given value, and skip any repeats.
...
| Two pointers + map (Step 3) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| `strconv` key (Step 4) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
| Inline skip (Step 5) | <!-- TODO --> | <!-- TODO --> | <!-- TODO --> |
...
// ... one Benchmark function per version ...
```
...
`-benchmem` adds the `B/op` and `allocs/op` columns, and `-count=3` runs each benchmark three times so you can eyeball stability.
...
Results on <!-- TODO: CPU model --> (averages over <!-- TODO: N --> runs, 1000 random inputs cycled per benchmark):

<!-- TODO: fill in the table above Step 5 with results, then add a sentence here summarizing the speedup and allocation reduction -->
